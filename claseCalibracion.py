...
def guardar_calibracion(tipoCanal,tipoCurva,parametros,covarianzas,dosis,transmitancias,ceros,incertidumbres,funcionesRGB,funcionCalDel,funcionTaD,labels,nombreArchivo):
    """Saves a calibration as a .calib file with all the information of calibration
    
    WARNING! Dill persitance may not be secure
    """
...
def leer_Calibracion(nombreArchivo):
    """Read a calib file and return information"""
    f=open(nombreArchivo,'rb')
    return dill.load(f)
    
class CalibracionImagen:
    """Main class for the calibration processes"""
    ...
    def generar_calibracion(self,nombreArchivo):
        ...
        if self.tipoCanal=="Multicanal":
            ...
            self.labelR=r"$a={0:.2g} \pm {1:.2g}$".format(self.parametrosOptimosR[0],self.pCovarianzaR[0])+'\n'+r"$b={0:.2g} \pm {1:.2g}$".format(self.parametrosOptimosR[1],self.pCovarianzaR[1])+'\n'+r"$c={0:.2g} \pm {1:.2g}$".format(self.parametrosOptimosR[2],self.pCovarianzaR[2])
            self.labelG=r"$a={0:.2g} \pm {1:.2g}$".format(self.parametrosOptimosG[0],self.pCovarianzaG[0])+'\n'+r"$b={0:.2g} \pm {1:.2g}$".format(self.parametrosOptimosG[1],self.pCovarianzaG[1])+'\n'+r"$c={0:.2g} \pm {1:.2g}$".format(self.parametrosOptimosG[2],self.pCovarianzaG[2])
            self.labelB=r"$a={0:.2g} \pm {1:.2g}$".format(self.parametrosOptimosB[0],self.pCovarianzaB[0])+'\n'+r"$b={0:.2g} \pm {1:.2g}$".format(self.parametrosOptimosB[1],self.pCovarianzaB[1])+'\n'+r"$c={0:.2g} \pm {1:.2g}$".format(self.parametrosOptimosB[2],self.pCovarianzaB[2])
        ...
        if self.tipoCanal=="Canal solo":
            ...
            def deltaOptimo(oDR,oDG,oDB):
                """Newton method for multichannel processing"""
                x0=0*oDR+1
                N=20
                for i in range(N):
